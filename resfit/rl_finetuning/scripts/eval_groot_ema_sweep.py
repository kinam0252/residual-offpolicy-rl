...
EMA_VALUES = [0.0, 0.3, 0.5, 0.7, 0.9]
NE = 10; NEP = 5  # 10 envs x 5 batches = 50 episodes
SEED = 42
...
print("=" * 60)
print("GR00T-only EMA sweep | Fixed seed={} | {} episodes each".format(SEED, NE*NEP))
print("=" * 60, flush=True)

for ema in EMA_VALUES:
    # Reset seed for each EMA value so cube positions are identical
    np.random.seed(SEED)
    torch.manual_seed(SEED)

    me = MuJoCoVecEnv(num_envs=NE, random_cube_range=rr, scene_xml=scene_xml,
    ...
    for ep in range(NEP):
        ...
        print(f"  EMA={ema} Batch {ep}: SR={sr:.0%} {es}", flush=True)

    print(f"  >>> EMA={ema}: SR={ts/te:.1%} ({ts}/{te})")
    print("-" * 40, flush=True)

    # Cleanup to free GPU memory before next EMA
    del env, me
