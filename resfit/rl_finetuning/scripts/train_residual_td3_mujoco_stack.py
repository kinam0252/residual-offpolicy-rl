"""
Residual TD3 training on MuJoCo Stack Cube + GR00T.

Standalone script using ``MuJoCoVecEnvStack`` + ``MuJoCoResidualWrapperStack``
for Stack Cube task.  Gripper uses raw metres [0, 0.04], NOT normalised.

Launch:
    source ~/.venvs/groot/bin/activate
    MUJOCO_GL=egl LD_LIBRARY_PATH=~/.local/lib/gl:${LD_LIBRARY_PATH:-} \\
    python resfit/rl_finetuning/scripts/train_residual_td3_mujoco_stack.py \\
        --groot_checkpoint ~/DATA/INTERN/training/groot_stack_sim_66ep/checkpoint-100000 \\
        --num_envs 30 --total_timesteps 500000
"""
...
def _load_offline_data(
    ...
    Args:
        reward_relabel: "none" = use saved/computed reward (clamped to [0,1]),
    ...
    # Shuffle files so buffer gets a random sample (FIFO evicts oldest)
    import random as _rng
    _rng.shuffle(npz_files)

    # Load reward config for feature-based computation
    ...
    _computed_from_features = 0

    for npz_path in npz_files:
        # Stop once buffer is full — no point loading more (FIFO evicts anyway)
        if total_loaded >= max_transitions:
            _log(f"  [offline] Buffer full ({total_loaded} >= {max_transitions}), stopping early "
                 f"({len(npz_files) - npz_files.index(npz_path)} files skipped)")
            break
        data = np.load(str(npz_path), allow_pickle=True)
        ...
        if reward_relabel == "sparse":
            ...
            reward_np = compute_reward_from_features(features, config=reward_cfg, use_next_step=True)
            reward_t = torch.from_numpy(reward_np).clamp(0.0, 1.0)
            _computed_from_features += n
        else:
            # Use saved reward
        ...
        if total_loaded % 50000 < n:
            _log(f"  ... {total_loaded} transitions loaded so far ({len(npz_files)} files)")

    _log(f"  Loaded {total_loaded} transitions from {len(npz_files)} files")
    if _computed_from_features > 0:
...
class AsyncEvaluator:
    ...
    def start(self, wandb_run_id: str | None = None):
        ...
        eval_script = str(Path(__file__).parent / "eval_async_mujoco_stack.py")
        a = self._args
        ...
        if hasattr(a, "eval_positions_file") and a.eval_positions_file:
            ...
            cmd += ["--white_cube_pos"] + [str(x) for x in a.white_cube_pos]
            cmd += ["--num_envs", str(a.num_envs)]
        if a.scene_xml:
            cmd += ["--scene_xml", a.scene_xml]
        if a.calib_path:
            cmd += ["--calib_path", a.calib_path]
        if not False and not (hasattr(a, "eval_positions_file") and a.eval_positions_file) and a.random_cube_range:
            cmd += ["--random_cube_range", a.random_cube_range]
        pass  # Stack: no bowl range
        # Only forward episode_positions_file when no fixed eval mode is active
        _has_fixed_eval = (hasattr(a, "eval_positions_file") and a.eval_positions_file) or False
        ...
        _log(f"[AsyncEval-Stack] Started PID={self._proc.pid}, log={log_path}")
...
def parse_args():
    p = argparse.ArgumentParser(description="Residual TD3 on MuJoCo Stack Cube + GR00T")
    # Environment
    ...
    p.add_argument("--episode_positions_file", type=str, default=None,
                   help="JSON with per-episode white/green cube positions")
    ...
    p.add_argument("--eval_positions_file", type=str, default=None,
                   help="Fixed eval positions JSON")
    ...
    # Residual scales (grip_scale in raw metres for Stack)
    ...
    # Algorithm (defaults from PnP best: as=0.1, gamma=0.95, of=0.75, L2=1.0)
    ...
    p.add_argument("--reward_config", type=str, default=None,
                   help="YAML reward config for computing reward from features (configs/reward_stack.yaml)")
...
def main():
    ...
    _log("Creating MuJoCo Stack Cube environment...")

    # Load positions from JSON if provided
    white_positions = [args.white_cube_pos] * args.num_envs
    green_positions = [args.green_cube_pos] * args.num_envs

    if args.episode_positions_file:
        ...
        for p in pos_list:
            if p["episode"] not in seen:
                seen.add(p["episode"])
                unique_pos.append(p)
        white_positions = [p["white_cube_pos"] for p in unique_pos]
        green_positions = [p["green_cube_pos"] for p in unique_pos]
        if args.num_envs == 1:
            args.num_envs = len(unique_pos)
        _log(f"Loaded {len(unique_pos)} positions from {args.episode_positions_file}")
    ...
    if args.random_cube_range:
        ...
        _log(f"Random cube range: {args.random_cube_range}")
    ...
    if _calib_path is None:
        ...
        if "66ep" in ckpt_lower or "100ep" in ckpt_lower:
            ...
            if Path(_calib_66ep).exists():
                ...
                _log(f"Auto-detected 66ep checkpoint -> using {_calib_66ep}")
