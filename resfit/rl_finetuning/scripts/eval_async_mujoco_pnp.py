...
# ΓöÇΓöÇ Patch huggingface_hub repo_id validator ΓöÇΓöÇ
try:
...
def evaluate(
    ...
    for ep in range(num_episodes):
        ...
        for step in range(env.vec_env.max_episode_steps):
            ...
            # and got auto-reset ΓÇö we already captured them). For first step
...
def parse_args():
    ...
    p.add_argument("--success_threshold", type=float, default=0.095)
...
def main():
    ...
    if hasattr(args, "random_cube_range") and args.random_cube_range:
        ...
        if isinstance(_raw_rcr, list):
            random_cube_range = [_cvt_one(item) for item in _raw_rcr]
        else:
            random_cube_range = _cvt_one(_raw_rcr)
        _log(f"Random cube range: {random_cube_range}")

    # ΓöÇΓöÇ Auto-detect camera calibration from GR00T checkpoint ΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Create agent (same architecture) ΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ W&B (log to same run as training) ΓöÇΓöÇ
    _wb = None
    ...
    # ΓöÇΓöÇ Main polling loop ΓöÇΓöÇ
