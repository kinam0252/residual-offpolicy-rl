...
class MuJoCoEnvConfig:
    num_envs: int = 1
    # Cube
    cube_pos: list[float] = field(default_factory=lambda: [0.42, -0.03, 0.02])
    cube_yaw_deg: float = 0.0
    cube_size: list[float] = field(default_factory=lambda: [0.04, 0.02, 0.02])
    # Scene
    scene_xml: str | None = None
    calib_path: str | None = None
    # Episode
    max_episode_steps: int = 300
    success_threshold: float = 0.03  # metres (v31a uses 3cm)
    reward_type: str = "dense_clipped"  # "sparse" | "dense" | "dense_clipped"
    # Rendering
    rl_img_size: int = 84
    device: str = "cuda:0"
    # Residual scales
...
class ResidualTD3MuJoCoConfig:
    ...
    # Network (v31a defaults: actor=512, critic=1024, asymmetric)
    ...
    wandb: WandBConfig = field(
        default_factory=lambda: WandBConfig(
            project="mujoco-franka-pnp-residual-td3",
            mode="online",
        )
    )
