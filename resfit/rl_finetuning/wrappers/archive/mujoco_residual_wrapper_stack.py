...
New experiments should use: train_residual_td3_mujoco.py --task stack

Residual environment wrapper for MuJoCo Stack Cube + GR00T.
...
3. Residual is applied on top of the base absolute action:
   ...
   - ``combined_grip = clamp(base_grip + residual_grip × scale, 0, 0.04)``
4. The combined absolute target is passed to ``MuJoCoVecEnvStack.step()``.

Key difference from PnP wrapper: gripper is in raw meters [0, 0.04],
NOT normalised [0, 1].
"""
...
# Gripper range in raw meters (Stack Cube uses raw finger width, not normalised)
_GRIP_MIN = 0.0
_GRIP_MAX = 0.04


class MuJoCoResidualWrapperStack:
    """Combines GR00T base policy with residual RL on top of MuJoCoVecEnvStack.
    ...
    def __init__(
        ...
        task_description: str = "Pick up the white cube and stack it on the green cube.",
        action_horizon: int = 16,
        open_loop_horizon: int = 16,
        residual_pos_scale: float = 0.02,   # metres
        residual_rot_scale: float = 0.05,   # radians
        residual_grip_scale: float = 0.004, # metres (raw grip, NOT normalised)
        ...
        if action_scaler is not None:
            print("[MuJoCoResidualWrapperStack] Using ActionScaler mode (data-driven pos+grip normalization)")
        ...
        if self._skip_groot or Gr00tPolicy is None:
            self.policy = None
            print("[MuJoCoResidualWrapperStack] GR00T policy SKIPPED (returns zeros)")
        else:
            print(f"[MuJoCoResidualWrapperStack] Loading GR00T from {groot_checkpoint}...")
            t0 = time.time()
            ...
            print(f"[MuJoCoResidualWrapperStack] GR00T loaded in {time.time() - t0:.1f}s")
    ...
    def step(
        ...
        combined, combined_naction_7d = self._combine_actions(base_action, residual_np)  # (N, 8), (N,7)|None
    ...
    def _combine_actions(
        ...
        Returns (N, 8) combined absolute action and optionally (N, 7) normalized
        combined action for replay buffer (when ActionScaler is active).
        Gripper clamped to [0, 0.04] (raw metres, NOT normalised).
        """
        ...
        if self.action_scaler is not None:
            ...
            for i in range(self.num_envs):
                ...
                combined[i, 7] = np.clip(combined_pg_np[3], _GRIP_MIN, _GRIP_MAX)
                ...
                # Build 7D normalized combined for replay: [pos_norm(3), euler_res(3), grip_norm(1)]
            ...
            for i in range(self.num_envs):
                ...
                # Gripper: additive, clamped to raw meters range
    ...
    def _augment_obs(
        ...
        for QAgent compatibility. When ActionScaler is active, normalizes
        pos+grip and zeros euler.
        """
        out = dict(raw_obs)
        ...
        for i in range(base_action.shape[0]):
            ...
            ba_7d[i, 6] = base_action[i, 7]  # grip (raw metres)
    ...
    def root_state_w(self):
        """Return white cube root state (N, 7+) for training script compatibility."""
        states = []
        for i in range(self.num_envs):
    ...
    def _initial_cube_z(self):
        """Return initial white cube z for training script compatibility."""
