"""
Worker: Replay CSV episodes in IsaacLab and extract cube 6D pose.
Saves cube_pose per episode as .npz files.

Output:
  {output_dir}/data/chunk-000/episode_XXXXXX.parquet         (same as depth version)
  {output_dir}/videos/chunk-000/{image,right_image,wrist_image}/episode_XXXXXX.mp4
  Parquet columns include:
    - cube_pos: [x, y, z] float32  — cube world position
    - cube_quat_wxyz: [w, x, y, z] float32  — cube quaternion in Isaac convention
    (EE pose is already in the 'state' column: eef_pos(3) + eef_quat_xyzw(4) + gripper(2))

Usage:
  isaaclab.sh -p replay_all_worker_with_cube_pose.py --headless --enable_cameras --num_envs 1 \
...
parser.add_argument("--retry_episodes_json", type=str, default=None,
                    help="JSON file with list of episode indices to retry")
parser.add_argument("--friction", type=float, default=50000.0,
                    help="Gripper+cube friction")
parser.add_argument("--reward_type", type=str, default="dense_clipped",
                    choices=["dense", "sparse", "dense_clipped"],
                    help="Reward type")
parser.add_argument("--skip_video", action="store_true",
                    help="Skip video saving (faster, pose-only)")
...
def compute_reward(robot, cube, left_finger_idx, right_finger_idx,
                   hand_joint_ids, initial_cube_z, scene, N=1):
    ...
    try:
        ...
        if not hasattr(compute_reward, '_finger_sensor_ids'):
            ...
            compute_reward._finger_sensor_ids = [
                i for i, n in enumerate(body_names) if 'finger' in n.lower()
            ]
            if not compute_reward._finger_sensor_ids:
                compute_reward._finger_sensor_ids = list(
                    range(max(0, len(body_names)-2), len(body_names)))
            print(f"[compute_reward] Contact sensor finger IDs: "
                  f"{compute_reward._finger_sensor_ids} from {body_names}")
        if compute_reward._finger_sensor_ids:
            finger_forces = forces[:, compute_reward._finger_sensor_ids, :]
            contact_force_val = float(
                finger_forces.sum(dim=1).norm(dim=-1)[0].item())
    ...
    if REWARD_TYPE == "sparse":
        ...
        return reward, is_success, {
            "reward_total": reward,
            "reward_distance": 0.0, "reward_grasp": 0.0,
            "reward_height": 0.0, "reward_success": reward,
            "cube_h": cube_height.item(), "dist": finger_cube_dist.item(),
            "grasp": 0.0, "fj": 0.0,
    ...
    return reward, is_success, {
        ...
        "reward_grasp": (grasp_gate * 2.0).item()
            if REWARD_TYPE == "dense" else (grasp_gate * 0.2).item(),
...
def reset_scene(robot, cube, scene, sim, sim_dt, csv_dir,
                all_joint_ids, hand_joint_ids, N=1):
    js = pd.read_csv(csv_dir / "franka_joint_states.csv",
                     header=0).values.astype(np.float32)
    gs = pd.read_csv(csv_dir / "gripper_joint_states.csv",
                     header=0).values.astype(np.float32)
    js_data = js[1:, 10:17]
    gs_data = gs[1:, 4:5]
    ...
    q_init = np.concatenate(
        [arm_init, [finger_val] * len(hand_joint_ids)]
    ).astype(np.float32)
    ...
    if object_pos_csv.exists():
        od = pd.read_csv(object_pos_csv,
                         header=0).values.astype(np.float32)
        obj = od[0, 3:7].copy()
    else:
        obj = np.array([0.2, 0.0, 0.06, 0.0], dtype=np.float32)
    env_origins = scene.env_origins
    obj_pos_w = env_origins + torch.tensor(
        obj[:3], device="cuda:0").unsqueeze(0)
    default_rot = cube.data.default_root_state[0, 3:7].cpu().numpy()
    yaw_quat = _yaw_to_quat_wxyz(float(obj[3]))
    base_r = Rotation.from_quat(
        [default_rot[1], default_rot[2], default_rot[3], default_rot[0]])
    yaw_r = Rotation.from_quat(
        [yaw_quat[1], yaw_quat[2], yaw_quat[3], yaw_quat[0]])
    cq = (yaw_r * base_r).as_quat()
    obj_quat_w = torch.tensor(
        [[cq[3], cq[0], cq[1], cq[2]]],
        device="cuda:0", dtype=torch.float32)
    cube.write_root_pose_to_sim(
        torch.cat([obj_pos_w, obj_quat_w], dim=-1))
...
def main():
    ...
    all_csv_dirs = sorted([
        d for d in csv_base.parent.iterdir()
    ...
    if args_cli.retry_episodes_json:
        ...
        print(f"[Worker {wid}] Retry mode: {len(retry_set)} episodes")
    ...
    print(f"[Worker {wid}] Processing episodes {start}-{end-1} "
          f"({len(my_dirs)} episodes out of {total})")

    # Create sim + scene (no depth needed, just RGB for videos)
    sim_cfg = sim_utils.SimulationCfg(dt=sim_dt, device="cuda:0")
    sim = sim_utils.SimulationContext(sim_cfg)
    scene_cfg = _make_scene_cfg(num_envs=N)
    # Only RGB (no depth needed for pose extraction)
    scene = InteractiveScene(scene_cfg)
    sim.reset()

    # Camera warmup
    env_origins = scene.env_origins
    eye_front = env_origins + torch.tensor(
        [[0.4, -0.7, 0.8]], device="cuda:0")
    eye_back = env_origins + torch.tensor(
        [[0.4, 0.7, 0.8]], device="cuda:0")
    target_pos = env_origins + torch.tensor(
        [[0.25, 0.0, -0.05]], device="cuda:0")
    for _ in range(10):
        scene.write_data_to_sim()
        sim.step()
        scene.update(sim_dt)
    ...
    arm_jn = sorted([
        ...
        if "panda_joint" in n and "finger" not in n
    ])
    arm_ji = [robot.data.joint_names.index(n) for n in arm_jn]
    hand_jn = sorted([
        n for n in robot.data.joint_names if "panda_finger_joint" in n
    ])
    ...
    robot.root_physx_view.set_material_properties(
        rm, torch.arange(N, device="cpu", dtype=torch.long))
    cm = cube.root_physx_view.get_material_properties().to("cpu")
    cm[..., 0] = friction_val
    cm[..., 1] = friction_val
    cube.root_physx_view.set_material_properties(
        cm, torch.arange(N, device="cpu", dtype=torch.long))
    print(f"[Worker {wid}] Friction set to {friction_val}")
    ...
    if not args_cli.skip_video:
        for vn in ["right_image", "image", "wrist_image"]:
            (video_base / vn).mkdir(parents=True, exist_ok=True)
    # No separate cube_pose dir needed — pose goes directly into parquet
    ...
    for ep_local, csv_dir in enumerate(my_dirs):
        ...
        if (data_dir / f"{ep_name}.parquet").exists():
            continue
        ...
        delta_csv = pd.read_csv(
            delta_csv_path, header=0).values.astype(np.float32)
        T = min(len(js_data), len(gs_data), len(delta_csv))

        states, actions = [], []
        rewards_list, reward_components, timestamps = [], [], []
        vid_frames = {k: [] for k in cameras}
        # Cube 6D pose buffers
        cube_positions = []      # (T, 3)
        cube_quats_wxyz = []     # (T, 4)
        total_reward = 0.0
        success_step = -1
        ...
        for t in range(T):
            ...
            q_target = np.concatenate(
                [arm_pos, [finger_val] * len(hand_ji)]
            ).astype(np.float32)
            q_target_t = torch.tensor(
                q_target, device="cuda:0").unsqueeze(0)
            ...
            for sub_step in range(steps_per_csv_row):
                ...
                robot.set_joint_position_target(
                    smoothed_target, joint_ids=all_ji)
                scene.write_data_to_sim()
                sim.step()
                scene.update(sim_dt)

            # Update cameras (for video)
            if not args_cli.skip_video:
            ...
            reward, is_success, dbg = compute_reward(
                robot, cube, left_finger_idx, right_finger_idx,
                hand_ji, initial_cube_z, scene)
            total_reward += reward
            if is_success and success_step < 0:
                success_step = t

            # === Extract cube 6D pose ===
            cube_state = cube.data.root_state_w[0]  # (13,): pos(3) quat(4) vel(3) angvel(3)
            cube_pos_np = cube_state[:3].detach().cpu().numpy().astype(np.float32)
            cube_quat_wxyz_np = cube_state[3:7].detach().cpu().numpy().astype(np.float32)
            cube_positions.append(cube_pos_np)
            cube_quats_wxyz.append(cube_quat_wxyz_np)

            # Build state: EEF pos(3) + quat_xyzw(4) + gripper_qpos(2) = 9D
            ee_pos = robot.data.body_pos_w[
                0, hand_body_idx].detach().cpu().numpy()
            ee_quat_wxyz = robot.data.body_quat_w[
                0, hand_body_idx].detach().cpu().numpy()
            ee_quat_xyzw = np.array([
                ee_quat_wxyz[1], ee_quat_wxyz[2],
                ee_quat_wxyz[3], ee_quat_wxyz[0]
            ])
            gripper_qpos = robot.data.joint_pos[
                0, hand_ji].detach().cpu().numpy()
            state_9d = np.concatenate(
                [ee_pos, ee_quat_xyzw, gripper_qpos]
            ).astype(np.float32)
            states.append(state_9d)

            # Build normalized base action
            raw_action = (delta_csv[t, :7].astype(np.float32)
                          if t < len(delta_csv)
        ...
        # --- Save parquet (with cube 6D pose included) ---
        records = []
        for i in range(len(states)):
            ...
            records.append({
                ...
                "cube_pos": cube_positions[i].tolist(),
                "cube_quat_wxyz": cube_quats_wxyz[i].tolist(),
        ...
        print(f"[W{wid}] {ep_local+1}/{len(my_dirs)} ep={ep_global} "
              f"steps={len(states)} R={total_reward:.1f} "
              f"succ={success_step >= 0} "
              f"cube_pose=({cube_positions[-1][0]:+.3f},"
              f"{cube_positions[-1][1]:+.3f},"
              f"{cube_positions[-1][2]:+.3f}) "
    ...
    print(f"[Worker {wid}] Done: {len(summary)} episodes, "
          f"{n_succ} successes ({100*n_succ/max(1,len(summary)):.1f}%)")
    os._exit(0)
