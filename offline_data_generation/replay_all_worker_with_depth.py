"""
Worker: Replay CSV episodes in IsaacLab with DEPTH capture (no GR00T).
Saves data, RGB videos, and depth images.
...
Output:
  ...
  {output_dir}/depth_front/episode_XXXXXX.npz   (raw depth meters, (T,84,84) float32)
  {output_dir}/depth_wrist/episode_XXXXXX.npz

Usage:
  isaaclab.sh -p replay_all_worker_with_depth.py --headless --enable_cameras --num_envs 1 \
...
def compute_reward(robot, cube, left_finger_idx, right_finger_idx, hand_joint_ids, initial_cube_z, scene, N=1):
    ...
    grasp_gate = has_contact  # Match online env: contact force alone gates reward
...
def main():
    ...
    print(f"[Worker {wid}] Cameras patched for RGB + Depth")
    ...
    for ep_local, csv_dir in enumerate(my_dirs):
        ...
        if (data_dir / f"{ep_name}.parquet").exists() and \
           (depth_front_dir / f"{ep_name}.npz").exists() and \
           (depth_wrist_dir / f"{ep_name}.npz").exists():
            continue
        ...
        for t in range(T):
            ...
            # Capture depth (raw meters, resized to 84x84)
            for cam_obj, frame_list in [
                (scene["camera_front"], depth_front_frames),
                (scene["camera_wrist"], depth_wrist_frames),
            ]:
                d = cam_obj.data.output["depth"][0].cpu().numpy().squeeze()
                d = np.nan_to_num(d, nan=10.0, posinf=10.0, neginf=0)
                d_t = torch.tensor(d).unsqueeze(0).unsqueeze(0).float()
                d_84 = F.interpolate(d_t, size=(84, 84), mode="bilinear", align_corners=False)
                frame_list.append(d_84[0, 0].numpy().astype(np.float32))
        ...
        # Save depth
        if depth_front_frames:
            np.savez_compressed(str(depth_front_dir / f"{ep_name}.npz"),
                               frames=np.stack(depth_front_frames).astype(np.float32))
        if depth_wrist_frames:
            np.savez_compressed(str(depth_wrist_dir / f"{ep_name}.npz"),
                               frames=np.stack(depth_wrist_frames).astype(np.float32))
