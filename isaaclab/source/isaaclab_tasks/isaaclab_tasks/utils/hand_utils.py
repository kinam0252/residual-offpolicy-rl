...
class ReferenceTrajInfo:
    def __init__(self, num_envs, device):
        ...
        self.subtasks_span = {
            "approach": [0.0, 0.3], "grasp": [0.3, 0.7], "pick": [0.7, 1.0],
        }
        self._pick_diff = torch.tensor([0., 0., 0.2], device=self.device).unsqueeze(0)
...
class HondaHandUtils:
    def __init__(self, grasp_type):
        self.grasp_type = grasp_type
        self.ik_target_link = "RHand_WRY_LINK" #"RHand_FOREARM_ROOT_LINK"
        self.shape_joint = [0, 1.5, 0.3, 0.2, 1.53, 1.5, 0.8, 0.2]
    ...
    def couplingRule(self, jv):
        ...
        ff3 = copy.deepcopy(jv[1])
        ff2 = copy.deepcopy(jv[2])
        ff1 = -0.00967753063 * ff2**5 + 0.0490372761 * ff2**4 + 0.0150052231 * ff2**3 + 0.125130786 * ff2**2 + 0.604870348 * ff2
        th5 = copy.deepcopy(jv[-4])
        th4 = copy.deepcopy(jv[-3])
        th2 = copy.deepcopy(jv[-2])
        th1 = -0.00792793444 * th2 ** 5 + 0.0354843085 * th2 ** 4 + 0.0208744889 * th2 ** 3 + 0.108985669 * th2 ** 2 + 0.639673417 * th2
        js = [ff4, ff3, ff2, ff1, 0, ff3, ff2, ff1, ff4, ff3, ff2, ff1, th5, th4, th2, th1]
        return js
