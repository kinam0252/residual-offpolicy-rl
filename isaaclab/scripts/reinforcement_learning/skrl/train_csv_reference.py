...
parser.add_argument(
    "--data_dir",
    type=str,
    default="/home/kinam/Desktop/DATA/dataset_from_Namiko/0_Raw_dataset/pickMushroom",
    help="CSV episode root folder. If --csv_base_dir is not set, this path is used.",
)
parser.add_argument(
    "--csv_episode",
    type=str,
    default=None,
    help="Optional episode subfolder under --data_dir. Example: pickMushroom_20251209_082334_546",
)
parser.add_argument(
    "--api_call_interval",
    type=int,
    default=16,
    help="API call interval override for env (default: 16 to match reference pipeline).",
)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    if csv_base_dir is None:
        csv_base_dir = os.path.join(args_cli.data_dir, args_cli.csv_episode) if args_cli.csv_episode else args_cli.data_dir
    if csv_base_dir is not None and hasattr(env_cfg, "csv_base_dir"):
        env_cfg.csv_base_dir = csv_base_dir
        print(f"[INFO] csv_base_dir set to: {csv_base_dir}")

    if hasattr(env_cfg, "reference_match_mode"):
        env_cfg.reference_match_mode = True
    if hasattr(env_cfg, "sim") and hasattr(env_cfg.sim, "dt"):
        env_cfg.sim.dt = 0.01
    if hasattr(env_cfg, "decimation"):
        env_cfg.decimation = 1
    if hasattr(env_cfg, "api_call_interval") and args_cli.api_call_interval is not None:
        env_cfg.api_call_interval = int(args_cli.api_call_interval)
    if hasattr(env_cfg, "use_wandb"):
    ...
    if hasattr(env_cfg, "reference_match_mode"):
        env_cfg.reference_match_mode = bool(args_cli.gr00t_reference_match)
    if bool(args_cli.gr00t_reference_match):
        if hasattr(env_cfg, "sim") and hasattr(env_cfg.sim, "dt"):
            env_cfg.sim.dt = 0.01
        if hasattr(env_cfg, "decimation"):
            env_cfg.decimation = 1
