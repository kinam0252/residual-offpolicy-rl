...
"""
GR00T-based motion generation with 3 cameras and PolicyClient (ZMQ).
...
parser.add_argument("--gr00t_host", type=str, default="127.0.0.1", help="GR00T policy server host.")
parser.add_argument("--gr00t_port", type=int, default=5555, help="GR00T policy server port.")
...
try:
    ...
    raise ImportError(
        "gr00t not found. Add Isaac-GR00T to PYTHONPATH or install dependencies in this env."
    )
...
# constants
# -----------------------------
GR00T_HOST = getattr(args_cli, "gr00t_host", "127.0.0.1")
GR00T_PORT = getattr(args_cli, "gr00t_port", 5555)
...
def run_simulator(
    ...
    # ---- GR00T client
    client = PolicyClient(host=GR00T_HOST, port=GR00T_PORT, strict=False)
    if not client.ping():
        raise RuntimeError(f"GR00T server ping failed at {GR00T_HOST}:{GR00T_PORT}. Start server first.")
    print(f"[INFO] GR00T client connected to {GR00T_HOST}:{GR00T_PORT}")

    # get action horizon from modality config
    mod_cfg = client.get_modality_config()
    try:
    ...
    try:
        while simulation_app.is_running():
            ...
            if steps_since_infer >= inference_interval:
                # build video dict with correct mapping:
                # wrist_view <- wrist, left_view <- back, right_view <- front
                # NOTE: Some GR00T configs expect 'ego_view' instead of 'wrist_view',
                # so we also provide an alias 'ego_view' pointing to the same data.
                video_dict = {}
                try:
                    ...
                    wrist_seq = wrist[None, None, ...].astype(np.uint8)
                    video_dict["wrist_view"] = wrist_seq
                    video_dict["ego_view"] = wrist_seq  # alias for configs trained with 'ego_view'
                    video_dict["left_view"] = left[None, None, ...].astype(np.uint8)
                    video_dict["right_view"] = right[None, None, ...].astype(np.uint8)
                except Exception as e:
                ...
                if video_dict is not None:
                    ...
                    state_dict = {
                        STATE_JOINT_KEY: joint_pos_np[None, None, :].astype(np.float32),
                        STATE_GRIPPER_KEY: np.array([[[gripper_frac]]], dtype=np.float32),
                    }
                    ...
                    try:
                        ...
                        print(f"[WARN] GR00T call failed: {e}")
...
def main():
    ...
    if csv_arg is not None:
        ...
        csv_dirs = sorted([p for p in data_root.iterdir() if p.is_dir()])
        if not csv_dirs:
            raise RuntimeError(f"No episode subdirectories found under data_dir={data_root}")

    out_root = Path(__file__).resolve().parents[2] / "outputs_gr00t"
    out_root.mkdir(parents=True, exist_ok=True)
    debug_root = out_root / "debug" if DEBUG_MODE else None

    print("[INFO] setup complete.")
    print(f"[INFO] server={GR00T_HOST}:{GR00T_PORT}")
    ...
    for ep_dir in csv_dirs:
        out_path = out_root / f"{ep_dir.name}.mp4"
        debug_dir = (debug_root / ep_dir.name) if debug_root is not None else None
        print(f"[INFO] Running episode for csv_dir={ep_dir} -> video={out_path}")
        if debug_dir is not None:
            print(f"[INFO] Debug videos (left/right/wrist) will be saved under: {debug_dir}")
        run_simulator(sim, scene, ep_dir, out_path, debug_dir)
