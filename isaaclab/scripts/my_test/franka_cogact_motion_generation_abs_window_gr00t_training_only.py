...
def main():
    ...
    for ep_dir in csv_dirs:
        ...
        for attempt in range(1, max(1, MAX_ATTEMPTS_PER_EPISODE) + 1):
            ...
            print(f"[INFO] Running episode={ep_dir.name} attempt={attempt}/{MAX_ATTEMPTS_PER_EPISODE} -> video={out_path}")
            if debug_dir is not None:
                print(f"[INFO] Debug videos (left/right/wrist) will be saved under: {debug_dir}")

            result = run_simulator(sim, scene, ep_dir, out_path, debug_dir)
        ...
        episode_row = {
            ...
            "max_attempts": int(MAX_ATTEMPTS_PER_EPISODE),
