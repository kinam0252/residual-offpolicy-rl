...
# CUDA_VISIBLE_DEVICES set by SLURM --gres=gpu:1
...
OUTPUT_DIR="outputs/mujoco_td3_v6_sparse/${TIMESTAMP}"

echo "=== MuJoCo TD3 v6: sparse reward + asymmetric ==="
echo "Job $SLURM_JOB_ID | $(hostname) | GPU: $(nvidia-smi --query-gpu=name,index --format=csv,noheader 2>/dev/null | head -1)"
...
python3 resfit/rl_finetuning/scripts/train_residual_td3_mujoco.py \
    ...
    --eval_num_episodes 5 \
    --seed 42 \
    --output_dir outputs/mujoco_td3/v6_sparse_asym_sub_ \
    --wandb_mode online \
    --wandb_project mujoco-franka-residual-td3 \
    --wandb_name "v6_sparse_asym_sub_${TIMESTAMP}"
