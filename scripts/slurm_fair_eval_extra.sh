...
#SBATCH --mem=80G
#SBATCH --job-name=fair_extra
#SBATCH --output=/home/nas_main/kinamkim/slurms/fair_extra_%j.out
#SBATCH --error=/home/nas_main/kinamkim/slurms/fair_extra_%j.err
...
OUT_BASE="outputs/pnp_fair_eval_ema09"
POS_FILE="configs/pnp_common5_positions.json"
NUM_RUNS=3
EMA_ALPHA=0.9

COMBOS=(
    "groot_pnp_sim_10ep 100000"
    "groot_pnp_sim_10ep 125000"
    "groot_pnp_sim_10ep 150000"
    "groot_pnp_sim_10ep 175000"
    "groot_pnp_sim_20ep 150000"
    "groot_pnp_sim_20ep 175000"
    "groot_pnp_sim_20ep 200000"
    "groot_pnp_sim_20ep 225000"
    "groot_pnp_sim_20ep 250000"
    "groot_pnp_sim_20ep 275000"
)

LOCK_TIMEOUT=3600

echo "[Worker $$] Starting FAIR eval EXTRA on $(hostname) at $(date)"
...
while true; do
    ...
    for combo in "${COMBOS[@]}"; do
        ...
        if mkdir "$lock_dir" 2>/dev/null; then
            ...
            echo ""
            echo "=========================================="
            echo "[Worker $$] ${short_name} step=${step_k}k EMA=$EMA_ALPHA"
            echo "=========================================="

            ckpt_path="${BASE_DIR}/${ckpt_name}/checkpoint-${ckpt_step}"
            if [ ! -d "$ckpt_path" ]; then
                echo "[WARN] Not found: $ckpt_path"
                rm -rf "$lock_dir"
                continue
            fi
...
import json, glob, os
srs = []
for rd in sorted(glob.glob('${out_dir}/run_*')):
    rf = os.path.join(rd, 'results.json')
    if os.path.exists(rf):
        srs.append(json.load(open(rf))['success_rate'])
if srs:
    avg = sum(srs)/len(srs)
    json.dump({'avg_sr': avg, 'run_srs': srs, 'num_runs': len(srs), 'ema_alpha': 0.9}, open('${out_dir}/fair_results.json','w'), indent=2)
    print(f'  AVG={avg*100:.1f}% runs={[round(s*100) for s in srs]}')
"
            rm -rf "$lock_dir"
