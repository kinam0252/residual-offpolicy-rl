...
OUT_BASE="/home/t-kinamkim/Repos/VLA_RL/residual-offpolicy-rl/outputs/unseen_eval_v16_all"
...
for f in $(ls "$CKPT_DIR"/agent_step*.pt | sort -Vr); do
    ...
    outdir="$OUT_BASE/v16_step${step}_unseen"
...
EOF
    ...
    while true; do
        ...
        for s in "${BATCH_STEPS[@]}"; do
            [ -f "$OUT_BASE/v16_step${s}_unseen/results.json" ] && done_n=$((done_n+1))
        done
    ...
    for s in "${BATCH_STEPS[@]}"; do
        rf="$OUT_BASE/v16_step${s}_unseen/results.json"
        if [ -f "$rf" ]; then
...
echo ""
echo "=== ALL RESULTS ==="
for f in $(ls "$OUT_BASE"/v16_step*_unseen/results.json 2>/dev/null | sort -t'p' -k2 -n); do
