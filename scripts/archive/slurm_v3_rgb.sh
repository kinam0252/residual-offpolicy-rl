...
OUTPUT_DIR="outputs/mujoco_td3_v3_rgb/${TIMESTAMP}"

echo "=== MuJoCo TD3 v3: RGB mode (no asymmetric) ==="
echo "Job ID: $SLURM_JOB_ID | Node: $(hostname) | GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader | head -1)"
echo "Config: RGB 3-cam, no asymmetric, dense_clipped, 10 envs, 50K steps"
...
python3 resfit/rl_finetuning/scripts/train_residual_td3_mujoco.py \
    ...
    --wandb_name "v3_rgb_3cam_${TIMESTAMP}"
