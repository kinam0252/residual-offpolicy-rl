...
OUTPUT_DIR="outputs/mujoco_td3_v5_seed123/${TIMESTAMP}"

echo "=== MuJoCo TD3 v5: Same as v2 but seed=123 ==="
echo "Job ID: $SLURM_JOB_ID | Node: $(hostname) | GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader | head -1)"
echo "Config: asymmetric, v31a config, seed=123"
...
python3 resfit/rl_finetuning/scripts/train_residual_td3_mujoco.py \
    ...
    --eval_num_episodes 5 \
    --seed 123 \
    --output_dir outputs/mujoco_td3/v5_asym_seed123_ \
    --wandb_mode online \
    --wandb_project mujoco-franka-residual-td3 \
    --wandb_name "v5_asym_seed123_${TIMESTAMP}"
