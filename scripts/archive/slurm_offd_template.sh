...
echo "=== Offline Data Collection env${ENV_IDX} ==="
echo "Job $SLURM_JOB_ID | $(hostname) | CUDA=$CUDA_VISIBLE_DEVICES"

python3 -c "import torch; torch.backends.cuda.enable_cudnn_sdp(False); print(f\"cuDNN SDP disabled, CUDA: {torch.cuda.get_device_name()}\")"
