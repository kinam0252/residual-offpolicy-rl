...
env = MuJoCoVecEnvCup(num_envs=1, episode_ids=[0], max_episode_steps=500,
...
for step in range(500):
    ...
    if step % 100 == 0:
        ...
        cup_z = data.qpos[e['cup_qposadr']+2]
        print(f"step={step} rew={reward.item():.3f} grip={grip:.4f} cup_z={cup_z:.4f} total_r={total_r:.1f}")

combined = [np.concatenate([fb, fw], axis=1) for fb, fw in zip(frames_base, frames_wrist)]
out = 'outputs/cup_base_videos/correction30_500steps.mp4'
