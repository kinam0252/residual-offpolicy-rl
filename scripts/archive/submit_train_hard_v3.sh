...
python3 resfit/rl_finetuning/scripts/train_residual_td3_mujoco_pnp.py \
    ...
    --output_dir "outputs/pnp_train_hard_v3/as${ACTION_SCALE}_of${OFFLINE_FRACTION}_cw${CRITIC_WARMUP}_ns${N_STEP}_l2${L2_REG}_s${SEED}" \
    --wandb_mode online \
    --wandb_project mujoco-franka-pnp-residual-td3 \
    --wandb_name "pnp_hard_v3_as${ACTION_SCALE}_of${OFFLINE_FRACTION}_cw${CRITIC_WARMUP}_ns${N_STEP}_l2${L2_REG}_s${SEED}" \
    --seed "$SEED"
