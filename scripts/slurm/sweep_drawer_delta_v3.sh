#!/bin/bash
#SBATCH --job-name=drawer_delta_sweep
#SBATCH --qos=share
#SBATCH --gres=gpu:1
...
# ══════════════════════════════════════════════════════════════════
# Drawer delta reward sweep — action_scale axis
# ckpt-75k, physics_drawer=True, D2+D3, 100k steps
#
# Usage: CONFIG_NAME=S1 sbatch scripts/slurm/sweep_drawer_delta_v3.sh
# ══════════════════════════════════════════════════════════════════
...
# Common: delta reward, L2=0.01, gamma=0.99, offline=0.3
L2=0.01; GAMMA=0.99; OFFLINE=0.3; REWARD=delta
LR_A=3e-4; LR_C=3e-4; WARMUP=1000; STEPS=500000

case "$CONFIG_NAME" in
    S1)  SCALE=0.05; TAG="delta_s005" ;;
    S2)  SCALE=0.10; TAG="delta_s010" ;;
    S3)  SCALE=0.15; TAG="delta_s015" ;;
    S4)  SCALE=0.20; TAG="delta_s020" ;;
    S5)  SCALE=0.30; TAG="delta_s030" ;;
    S6)  SCALE=0.50; TAG="delta_s050" ;;
    *)
