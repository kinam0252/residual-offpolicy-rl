...
TASKS = {
    ...
    "pick_and_place": {
        ...
        "rl_action_scale": 0.1, # from PnP training SLURM script (as=0.1)
    ...
    "stack_cube": {
        ...
        "rl_action_scale": 0.1,  # from B200 training (stack_td3_75k/hard)
        "residual_grip_scale": 0.004,  # from B200 training config
...
class ResidualActor(torch.nn.Module):
    ...
    def load_from_checkpoint(ckpt_path, device="cuda", action_scale=None):
        ...
        # Auto-read action_scale from checkpoint args if not explicitly given
        if action_scale is None and isinstance(ckpt, dict) and "args" in ckpt:
            ckpt_args = ckpt["args"]
            # args may be Namespace (hasattr) or dict (key lookup)
            if isinstance(ckpt_args, dict) and "action_scale" in ckpt_args:
                action_scale = ckpt_args["action_scale"]
            elif hasattr(ckpt_args, "action_scale"):
                action_scale = ckpt_args.action_scale
                print(f"  [ResidualActor] action_scale={action_scale} from checkpoint args")
...
def run(node: InferenceNode, args):
    ...
    if args.residual_checkpoint:
        residual_actor = ResidualActor.load_from_checkpoint(
    ...
    try:
        while rclpy.ok():
            ...
            if residual_actor is not None and use_residual_this_trial:
                with torch.no_grad():
                    for k in range(len(pred_pos)):
                        # Build state 10D: eef_pos(3) + eef_quat_xyzw(4) + gripper_qpos(2) + contact(1)
                        gripper_qpos_2d = np.array([gripper_width * 0.04, gripper_width * 0.04], dtype=np.float32)
                        contact_val = np.array([0.0], dtype=np.float32)  # no contact sensor yet
                        ...
                        # Object state 10D: cube pos(3) + quat_wxyz(4) + bowl_pos(3)
                        if use_fp:
                            fp_pos, fp_quat, fp_age = read_fp_pose(fp_max_age)
                            if fp_pos is not None:
                                obj_cube_7d = np.concatenate([fp_pos, fp_quat])
                                if fp_age > _FP_STALE_WARN_S:
                                    print(f"  [FP] pose stale: {fp_age:.1f}s", flush=True)
                            else:
                                obj_cube_7d = np.concatenate([cube_pos_init, cube_quat_wxyz_init])
                        else:
                            obj_cube_7d = np.concatenate([cube_pos_init, cube_quat_wxyz_init])
                        bowl_pos_3d = bowl_pos_init if bowl_pos_init is not None else np.array([0.42, 0.03, 0.0], dtype=np.float32)
                        obj_state_10d = np.concatenate([obj_cube_7d, bowl_pos_3d])
                        # Combined state 20D
                        state_20d = np.concatenate([state_10d, obj_state_10d]).astype(np.float32)
                        ...
                        ba_t = torch.as_tensor(ba_7d, device=residual_device).unsqueeze(0)
                        residual_7d = residual_actor(s_t, ba_t).squeeze(0).cpu().numpy()
                        # Zero out grip residual: B200 trained in 0~1 normalized,
                        # but real robot PnP uses raw meters — scale mismatch
                        if gripper_raw:
                            residual_7d[6] = 0.0
                        step_residuals.append(residual_7d.copy())
                        # Combine
                        pred_pos[k], pred_quat[k], pred_grip[k] = combine_actions(
                            pred_pos[k], pred_quat[k], pred_grip[k],
                            residual_7d,
                            args.residual_pos_scale,
                            args.residual_rot_scale,
                            args.residual_grip_scale,
                        )
