...
import argparse, glob, logging, os, queue, re, subprocess, sys, time, threading
...
def read_fp_stack_pose(max_age_s=2.0):
    ...
    Returns (pos_3, quat_wxyz_4, age_s) or (None, None, None).
    """
    try:
        with open(_FP_STACK_SHM, "r") as f:
            data = _json.load(f)
        if data.get("status") != "tracking":
            return None, None, None
        age = time.time() - data["timestamp"]
...
def signal_fp_stack_reinit():
    ...
    with open(_FP_STACK_REINIT, "w") as f:
...
TASKS = {
    ...
    "close_drawer": {
        ...
        # "residual_threshold": 0.04,  # disabled for testing — see if saturated residual helps
    },
    "stack_cube": {
...
def discover_and_select_checkpoints(ckpt_dir=CHECKPOINT_BASE_DIR,
    ...
    if os.path.isdir(rl_best_dir):
        for rl_sim_name in sorted(os.listdir(rl_best_dir)):
            ...
            if not os.path.isdir(rl_dir):
                continue
            # Two-level: rl_best/{rl_sim_name}/{variant}/checkpoints/best.pt
            found_two_level = False
            for variant in os.listdir(rl_dir):
                best_pt = os.path.join(
                    rl_dir, variant, "checkpoints", "best.pt")
                if os.path.exists(best_pt):
                    found_two_level = True
            ...
            # One-level fallback: rl_best/{variant}/checkpoints/best.pt
            if not found_two_level:
    ...
    if rl_options:
        ...
        if rl_choice != "none":
            ...
            # Read action_scale directly from checkpoint (authoritative)
            _ckpt = torch.load(residual_path, map_location="cpu", weights_only=False)
            _ckpt_args = _ckpt.get("args", {})
            if hasattr(_ckpt_args, "__dict__"):
                _ckpt_args = vars(_ckpt_args)
            ckpt_action_scale = _ckpt_args.get("action_scale", None)
            if ckpt_action_scale is not None:
                action_scale = ckpt_action_scale
                print(f"  -> {rl_choice['label']} (action_scale={action_scale} from checkpoint)")
            else:
                action_scale = rl_choice["action_scale"]
                print(f"  -> {rl_choice['label']} (action_scale={action_scale} from filename)")
...
def build_rl_state(task_key, real_ee_pos, real_ee_quat, gripper_width,
    ...
    if task_key == "cube_lift":
        ...
        # All values fixed for active drawer at episode start.
        ...
        DRAWER_FACE_CENTERS = {
            2: [0.5177, 0.1689, 0.1460],
            3: [0.5177, 0.1689, 0.2004],
            4: [0.5177, 0.1689, 0.2548],
        }
        active_idx = obj_data.get("active_drawer_idx", 2)
        face_center = DRAWER_FACE_CENTERS.get(active_idx, DRAWER_FACE_CENTERS[2])
        obj_state = np.array([
            0.13,              # HARDCODED: drawer_slide = fully open (episode start)
            float(active_idx), # active_drawer_idx
            face_center[0],    # face_center_world x
            face_center[1],    # face_center_world y
            face_center[2],    # face_center_world z
        ], dtype=np.float32)
    else:
...
def combine_actions_v2(base_pos, base_quat_xyzw, base_grip,
    ...
    Args:
        ...
        residual_threshold: if not None, skip residual (use base only)
            when norm(residual_7d) > threshold. Prevents saturated actors
            from destabilizing the robot.
    """
    from scipy.spatial.transform import Rotation

    # Safety: skip residual if saturated (norm too large)
    residual_skipped = False
    if residual_threshold is not None:
        res_norm = float(np.linalg.norm(residual_7d))
        if res_norm > residual_threshold:
            logging.warning(
                f"[RESIDUAL SKIP] norm={res_norm:.4f} > thresh={residual_threshold} — using base only"
            )
            residual_7d = np.zeros(7, dtype=np.float32)
...
# ── SAM2 + FoundationPose cube detection for stack_cube ──────────────
FP_DETECT_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                "fp_detect_stack.py")
FP_VENV_PYTHON = os.path.expanduser("~/kinam_dev/FoundationPose/venv/bin/python3")
FP_CUBE_4CM_MESH = os.path.expanduser("~/kinam_dev/FoundationPose/box_4x4x4cm.obj")

# SHM paths for IPC with subprocess
_SHM_STACK_RGB = "/dev/shm/stack_rgb.npy"
_SHM_STACK_DEPTH = "/dev/shm/stack_depth.npy"
_SHM_STACK_K = "/dev/shm/stack_K.npy"
_SHM_STACK_RESULT = "/dev/shm/stack_detect_result.json"


def detect_cubes_for_stack(node, calib_path, timeout=180):
    """Detect white + green cubes using SAM2+FP subprocess.

    Args:
        node: ROS node with latest_images, latest_depth, camera_intrinsics
        calib_path: path to calib_result_base.calib
        timeout: subprocess timeout in seconds

    Returns:
        dict with keys "white" and/or "green", each containing "pos" and "quat_wxyz",
        or empty dict on failure.
    """
    import rclpy as _rclpy

    print("\n" + "=" * 60)
    print("  STACK CUBE DETECTION (SAM2 + FoundationPose)")
    print("=" * 60)

    # Spin to get fresh frames
    print("  Capturing camera frame (RGB + Depth)...", flush=True)
    for _ in range(50):
        _rclpy.spin_once(node, timeout_sec=0.1)

    rgb = node.latest_images.get("base_rgb")
    depth = node.latest_depth
    intrinsics = node.camera_intrinsics

    if rgb is None:
        print("  [ERROR] No RGB image from cam_base!")
        return {}
    if depth is None:
        print("  [ERROR] No depth image from cam_base!")
        return {}

    print(f"  RGB: {rgb.shape}, Depth: {depth.shape}", flush=True)
    ...
    if intrinsics is not None:
        ...
        K = np.array([
            [_DEFAULT_INTRINSICS["fx"], 0, _DEFAULT_INTRINSICS["cx"]],
            [0, _DEFAULT_INTRINSICS["fy"], _DEFAULT_INTRINSICS["cy"]],
            [0, 0, 1],
        ], dtype=np.float64)
        print("  [WARN] Using default intrinsics (CameraInfo not received)")

    # Save to /dev/shm for subprocess
    np.save(_SHM_STACK_RGB, rgb)

    # Depth → float32 meters for FP (RealSense depth is uint16 in mm)
    depth_m = depth.astype(np.float32) / 1000.0
    np.save(_SHM_STACK_DEPTH, depth_m)

    np.save(_SHM_STACK_K, K)

    cmd = [
        FP_VENV_PYTHON, FP_DETECT_SCRIPT,
        "--rgb", _SHM_STACK_RGB,
        "--depth", _SHM_STACK_DEPTH,
        "--K", _SHM_STACK_K,
        "--calib", calib_path,
        "--mesh", FP_CUBE_4CM_MESH,
        "--output", _SHM_STACK_RESULT,
    ]
    ...
    print("  Running FP subprocess (SAM2 + FoundationPose)...")
    print("  (first run loads models, may take ~30-60s)", flush=True)
    ...
    try:
        ...
        if proc.returncode != 0:
            print(f"  [ERROR] Subprocess failed (rc={proc.returncode})")
            if proc.stderr:
        ...
        print(f"  [ERROR] Detection timed out ({timeout}s)")
        return {}

    # Parse result
    try:
        with open(_SHM_STACK_RESULT, "r") as f:
            result = _json.load(f)
    ...
    if result.get("status") != "ok":
        print(f"  [ERROR] Detection failed: {result.get('error', 'unknown')}")
        return {}

    cubes = result.get("cubes", {})
    print(f"\n  Detection complete in {result.get('elapsed_s', elapsed):.1f}s:")
    for color, info in cubes.items():
        pos = info["pos"]
        print(f"    {color:6s} cube: pos=[{pos[0]:.4f}, {pos[1]:.4f}, {pos[2]:.4f}]")

    if "white" not in cubes:
        print("  [WARN] White cube not detected!")
    if "green" not in cubes:
        print("  [WARN] Green cube not detected!")
...
class DebugLogger:
    ...
    def log_step(self, step, *, input_state, raw_output, safe_output,
                 ...
                 residual_skips=None, object_state=None,
        ...
        if residual_skips is not None:
            entry["residual_skips"] = residual_skips
        if object_state is not None:
            entry["object_state"] = object_state
...
class RosbagRecorder:
    """Record 7 ROS topics to a rosbag for later lerobot/FP conversion.

    Topics recorded:
      /franka_robot_state_broadcaster/current_pose  (PoseStamped, ~30Hz downsampled)
      /target_pose                                   (PoseStamped)
      /franka_gripper/joint_states                   (JointState, ~14Hz)
      /franka/joint_states                           (JointState, ~30Hz — needed for sim replay IK)
...
class InferenceNode(Node):
    ...
    def __init__(self, args):
        ...
        self.latest_depth = None  # uint16 aligned depth for FP detection
        ...
        try:
            ...
            if len(j) == 7:
                ...
                if self.rosbag_recorder:
                    ...
                    self.rosbag_recorder.push_ros_msg("joint_states", msg, ts)
        except Exception:
            pass
    ...
    def _cb_depth(self, msg):
        """Handle aligned depth image — store latest + forward to rosbag."""
        try:
            raw = np.frombuffer(msg.data, dtype=np.uint16).reshape(
                msg.height, msg.width)
            self.latest_depth = raw.copy()
        except Exception:
            pass
        if self.rosbag_recorder and self.rosbag_recorder._recording:
            try:
                ts = self.get_clock().now().nanoseconds
                self.rosbag_recorder.push_depth(self.latest_depth, ts)
            except Exception:
                pass
    ...
    def reset_sensor_timestamps(self):
        """Reset all sensor timestamps to now (call after long blocking ops like FP detection)."""
        now = time.time()
        for k in self._sensor_ts:
            self._sensor_ts[k] = now
...
def run(node: InferenceNode, args):
    ...
    # ── MuJoCo preview mode ──
    # Disabled by default; use --preview to enable.
    if args.preview:
    ...
    if args.residual_checkpoint:
        ...
        # Read raw checkpoint args for auto-detecting settings
        _ckpt_raw = torch.load(args.residual_checkpoint, map_location="cpu", weights_only=False)
        ckpt_args_raw = _ckpt_raw.get("args", {})
        if hasattr(ckpt_args_raw, "__dict__"):
            ckpt_args_raw = vars(ckpt_args_raw)
        del _ckpt_raw
        ...
        # Multi-drawer detection: check if checkpoint name suggests multi-drawer
        if task_key == "close_drawer" and args.residual_checkpoint:
            ckpt_name = os.path.basename(os.path.dirname(
                os.path.dirname(os.path.dirname(args.residual_checkpoint))))
            is_d2only = "d2only" in ckpt_name.lower()
            args._is_d2only_ckpt = is_d2only
            if is_d2only:
                args._active_drawer_idx = 2
                print(f"  Drawer mode: D2-only (auto-detected from checkpoint name)")
            else:
                args._active_drawer_idx = 2  # default, will be asked each trial
                print(f"  Drawer mode: MULTI-DRAWER ({ckpt_name}) — will ask D2/D3/D4 each trial")
    ...
    # ── Stack cube: FP detection helper (called on [r] mode or [f] re-detect) ──
    _stack_detected_cubes = {}

    def _run_stack_fp_detection():
        """Run SAM2+FP detection for stack_cube, update cube positions, launch viewer."""
        nonlocal cube_pos_init, cube_quat_wxyz_init, _stack_detected_cubes
        if task_key != "stack_cube":
            return {}
        _stack_detected_cubes = detect_cubes_for_stack(node, args.calib)
        ...
        # Launch overlay viewer as subprocess (use mujoco_viewer env for GUI opencv)
        # Kill previous viewer if running
        _prev_proc = getattr(args, '_stack_viewer_proc', None)
        if _prev_proc is not None:
            try:
                _prev_proc.terminate()
                _prev_proc.wait(timeout=3)
            except Exception:
                pass
            _prev_logf = getattr(args, '_stack_viewer_logf', None)
            if _prev_logf:
                try:
                    _prev_logf.close()
                except Exception:
        ...
        _stack_viewer_script = os.path.join(os.path.dirname(__file__),
                                            "fp_verify_stack.py")
        _viewer_python = os.path.expanduser(
            "~/miniconda3/envs/mujoco_viewer/bin/python3")
        if os.path.exists(_stack_viewer_script) and _stack_detected_cubes and \
                os.path.exists(_viewer_python):
            print("  [Stack] Launching overlay viewer...")
            _sv_env = os.environ.copy()
            _sv_env["DISPLAY"] = _sv_env.get("DISPLAY", ":1")
            _ros_lib = "/opt/ros/humble/lib:/opt/ros/humble/lib/x86_64-linux-gnu"
            _ros_py = "/opt/ros/humble/lib/python3.10/site-packages:/opt/ros/humble/local/lib/python3.10/dist-packages"
            _sv_env["LD_LIBRARY_PATH"] = _ros_lib + ":" + _sv_env.get("LD_LIBRARY_PATH", "")
            _sv_env["PYTHONPATH"] = _ros_py + ":" + _sv_env.get("PYTHONPATH", "")
            _sv_log = os.path.join(os.path.dirname(args.out_dir),
                                   "stack_viewer.log")
            _sv_logf = open(_sv_log, "w")
            args._stack_viewer_proc = subprocess.Popen(
    ...
    trial_dir = None
    _obj_state_log = []  # per-step object state tracking for data saving
    ...
    def finalize_trial():
        ...
        # Save object state log (per-step cube positions + rl_state)
        if _obj_state_log:
            obj_log_path = os.path.join(trial_dir, "object_state_log.json")
            with open(obj_log_path, "w") as _olf:
                _json.dump(_obj_state_log, _olf, indent=1)
            print(f"  Object state log: {obj_log_path} ({len(_obj_state_log)} steps)", flush=True)

        # Kill viewer + tracker so next trial gets fresh windows
        if task_key == "stack_cube":
            for _attr, _label in [("_stack_viewer_proc", "Viewer"),
                                  ("_stack_tracker_proc", "Tracker")]:
                _proc = getattr(args, _attr, None)
                if _proc is not None and _proc.poll() is None:
                    ...
                    try:
                        ...
                        _proc.kill()
                    print(f"  [Stack] {_label} (PID {_proc.pid}) stopped.", flush=True)
    ...
    print("Press [a] to start, [r] to return to init, "
          "[e]/[space] to E-STOP, [f] to re-detect FP, [Ctrl+C] to quit.", flush=True)
        ...
        while rclpy.ok():
            ...
            if key == 'g' and running:
                ...
                print("Press [r] to return to init, [a] to start new trial...",
                      flush=True)
                continue
            if key == 'f' and not running:
                # Re-run FP detection for stack_cube (between trials)
                if task_key == "stack_cube":
                    _tracker = getattr(args, '_stack_tracker_proc', None)
                    if _tracker is not None and _tracker.poll() is None:
                        # Tracker running — signal re-init (lightweight, no restart)
                        print("  [f] Signaling FP tracker to re-detect...", flush=True)
                        signal_fp_stack_reinit()
                        # Also re-run one-shot for green cube position update
                        _stack_detected_cubes = detect_cubes_for_stack(node, args.calib)
                        if "green" in _stack_detected_cubes:
                            g = _stack_detected_cubes["green"]
                            args.cube_b_pos = g["pos"]
                        ...
                        # No tracker running — full detection + launch
                        print("  [f] Re-running FP cube detection + tracker...", flush=True)
                        _run_stack_fp_detection()
                    print("  Press [a] to start trial, [f] to re-detect.", flush=True)
                else:
                    print("  [f] FP re-detect only available for stack_cube", flush=True)
                continue
                ...
                if residual_actor is not None:
                    ...
                    while mode_choice is None:
                        ...
                        if mk == "b":
                            ...
                            # Stack cube: run FP detection when residual is selected
                            if task_key == "stack_cube":
                                _run_stack_fp_detection()
                            # Multi-drawer: D2 or skip residual
                            if task_key == "close_drawer" and not getattr(args, '_is_d2only_ckpt', True):
                                print("\n  [2] D2 residual  [n] no residual (base only)")
                                drawer_choice = None
                                while drawer_choice is None:
                                    rclpy.spin_once(node, timeout_sec=0.01)
                                    dk = keys.get_key()
                                    if dk == "2":
                                        drawer_choice = 2
                                        ...
                                        drawer_choice = None
                                        use_residual_this_trial = False
                                        # Still save under "residual" folder
                                        print("  -> Base only (residual skipped)")
                                        break
                                if drawer_choice is not None:
                                    args._active_drawer_idx = drawer_choice
                                    print(f"  -> Active drawer: D{drawer_choice}")
            ...
            step_residual_skips = 0  # count of waypoints where residual was skipped
                ...
                with torch.no_grad():
                    for k in range(len(pred_pos)):
                        ...
                        obj_data = {
                            ...
                            # Multi-drawer: active_drawer_idx set during checkpoint selection
                            "active_drawer_idx": getattr(args, '_active_drawer_idx', 2),
                        }
                        ...
                        if _res_thresh is not None and np.linalg.norm(residual_7d) > _res_thresh:
                            step_residual_skips += 1
                        ...
                        pred_pos[k], pred_quat[k], pred_grip[k] = combine_actions_v2(
                            ...
                            residual_threshold=task_cfg.get("residual_threshold", None),
                        )
            ...
            debug_log.log_step(
                ...
                residual_skips=step_residual_skips if (use_residual_this_trial and step_residual_skips > 0) else None,
                object_state=_obj_state_log[-1] if _obj_state_log else None,
                safe_output={
...
def main():
    ...
    res_grp.add_argument("--cube-b-pos", type=float, nargs=3,
        default=[0.45, -0.06, 0.02],
        help="Green cube (B) fixed position xyz for stack task")
    ...
    vis_grp.add_argument("--viewer", dest="viewer", action="store_true", default=False,
        help="Enable MuJoCo live viewer (disabled by default)")
    ...
    try:
        ...
        if getattr(args, "_stack_viewer_proc", None) is not None:
            print("[Viewer] Shutting down stack overlay viewer...")
            args._stack_viewer_proc.terminate()
            args._stack_viewer_proc.kill(); args._stack_viewer_proc.wait(timeout=3)
            if getattr(args, "_stack_viewer_logf", None):
                args._stack_viewer_logf.close()
        if getattr(args, "_stack_tracker_proc", None) is not None:
            print("[Tracker] Shutting down stack FP tracker...")
            args._stack_tracker_proc.terminate()
            args._stack_tracker_proc.kill(); args._stack_tracker_proc.wait(timeout=3)
            if getattr(args, "_stack_tracker_logf", None):
                args._stack_tracker_logf.close()
        node.destroy_node()
        rclpy.shutdown()
